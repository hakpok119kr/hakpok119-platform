...
const aiOutputHistoryTypeOptions: Array<"all" | AiOutputType> = [
  "all",
  "case_summary",
  "case_analysis",
  "case_opinion",
...
export default function AdminPage() {
  ...
  const [deletingAiOutputHistoryId, setDeletingAiOutputHistoryId] = useState<string | null>(null);
  const [selectedAiOutputHistoryId, setSelectedAiOutputHistoryId] = useState<string | null>(null);
  const [aiOutputHistoryTypeFilter, setAiOutputHistoryTypeFilter] = useState<"all" | AiOutputType>("all");
  const [aiOutputHistorySearch, setAiOutputHistorySearch] = useState("");
  ...
  const selectedReservationFiles = selectedReservation?.id
    ...
    : [];
  const selectedAiOutputHistory = selectedReservation?.id
    ? aiOutputHistoryByReservation[selectedReservation.id] ?? []
    : [];
  ...
  useEffect(() => {
    ...
    void loadAiOutputHistory(selectedReservation.id);
  }, [dataSource, selectedReservation?.id]);

  useEffect(() => {
    setSelectedAiOutputHistoryId(null);
    setAiOutputHistoryTypeFilter("all");
    setAiOutputHistorySearch("");
  ...
  async function persistAiOutput(record: AiOutputRecord, title: string) {
    ...
    await loadAiOutputHistory(record.case_id);
    setMessage("AI 결과가 DB에 저장되었습니다.");
    return true;
  }

  async function loadAiOutputHistory(reservationId: string) {
    if (!reservationId || dataSource !== "supabase") {
      return;
    }

    setLoadingAiOutputHistoryId(reservationId);
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("ai_outputs")
        .select("id,reservation_id,output_type,title,model,content,created_at")
      ...
      const outputs = (data ?? []) as AiOutputHistory[];

      setAiOutputHistoryByReservation((current) => ({
        ...current,
        [reservationId]: outputs,
      }));
      setSelectedAiOutputHistoryId((current) => current ?? outputs[0]?.id ?? null);
    } catch (error) {
      console.error("Failed to load AI output history:", error);
      setMessage("AI 호출 이력 조회에 실패했습니다.");
    } finally {
      setLoadingAiOutputHistoryId((current) => (current === reservationId ? null : current));
    }
  }

  async function deleteAiOutputHistory(output: AiOutputHistory) {
    if (!window.confirm("선택한 AI 호출 이력을 삭제하시겠습니까?")) {
      return;
    }

    setDeletingAiOutputHistoryId(output.id);
    setMessage("");
    ...
    try {
      ...
      const { error } = await supabase.from("ai_outputs").delete().eq("id", output.id);

      if (error) {
        throw error;
      }

      setSelectedAiOutputHistoryId((current) => (current === output.id ? null : current));
      await loadAiOutputHistory(output.reservation_id);
      setMessage("AI 호출 이력을 삭제했습니다.");
    } catch (error) {
      console.error("Failed to delete AI output history:", error);
      setMessage("AI 호출 이력 삭제에 실패했습니다.");
    } finally {
      setDeletingAiOutputHistoryId(null);
    }
  }
  ...
  async function handleGenerateAiCaseAnalysisWithApi(caseId: string) {
    const reservation = reservations.find((item) => getReservationKey(item) === caseId);
    ...
    const incidentSummary = createPrivateSafeCaseText(
      [existingCaseSummary, reservation.content, reservation.admin_memo],
      privateValues,
    );
    const submittedDocuments = createPrivateSafeCaseText(
      [asText(reservation.submitted_documents), ...files.map((file) => file.file_name)],
      privateValues,
    );
    const existingOpinion = createPrivateSafeCaseText([aiCaseOpinions[caseId]], privateValues);
    const inputSnapshot = {
      caseStatus: removePrivateIdentifiers(reservation.case_status || "", privateValues),
      consultationContent,
      existingOpinion,
      incidentSummary,
      studentName: removePrivateIdentifiers(reservation.name || "", privateValues),
      submittedDocuments,
      userRole: getGeneralizedPartyLabel(reservation.student_type ?? reservation.studentRole),
    };

    setIsGeneratingCaseAnalysis(caseId);
    setCaseAnalysisError((current) => {
      const next = { ...current };
      delete next[caseId];
      return next;
    });
    ...
    try {
      ...
      if (!response.ok || !data.analysis) {
        const errorMessage = data.error ?? "AI 사건분석 생성에 실패했습니다.";
      ...
      const outputRecord: AiOutputRecord = {
        case_id: reservation.id || caseId,
        created_at: now,
        id: createAiOutputId(reservation.id || caseId, "case_analysis"),
        input_snapshot: inputSnapshot,
        model: "gpt-4o-mini",
        output_text: data.analysis,
        output_type: "case_analysis",
        prompt_version: "case_analysis_v1",
        updated_at: now,
      };

      setAiCaseAnalysis((current) => ({
        ...current,
          ...
          {selectedReservation ? (
            <div>
              ...
              <AiAssistantSection
                ...
                aiCaseAnalysis={aiCaseAnalysis[selectedReservationKey]}
              ...
              />
              <AiOutputHistorySection
                deletingId={deletingAiOutputHistoryId}
                filter={aiOutputHistoryTypeFilter}
                isLoading={loadingAiOutputHistoryId === selectedReservation.id}
                onCopy={(content) => navigator.clipboard.writeText(content)}
                onDelete={deleteAiOutputHistory}
                onFilterChange={setAiOutputHistoryTypeFilter}
                onRefresh={() => selectedReservation.id && loadAiOutputHistory(selectedReservation.id)}
                onSearchChange={setAiOutputHistorySearch}
                onSelect={setSelectedAiOutputHistoryId}
                outputs={selectedAiOutputHistory}
                search={aiOutputHistorySearch}
                selectedId={selectedAiOutputHistoryId}
              />
              <AccordionSection
...
function AiAssistantSection({
  ...
  const features = [
    ...
    {
      description: "상담기록, 사건상태, 제출자료, 기존 의견서를 바탕으로 객관적 사건분석을 생성합니다.",
      id: "caseAnalysis" as const,
      title: "AI 사건분석 생성",
    },
    {
...
function AiCaseAnalysisResult({
  ...
  return (
    <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4">
      <p className="text-sm font-black text-slate-900">AI 사건분석 결과</p>
      {isLoading ? (
        <p className="mt-3 text-sm font-bold text-blue-700">AI 사건분석을 생성하는 중입니다...</p>
      ) : null}
      {error ? (
        <p className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-bold text-red-700">
          AI 사건분석 생성에 실패했습니다.
      ...
      {value ? (
        <pre className="mt-3 max-h-[560px] overflow-auto whitespace-pre-wrap break-words rounded-xl border border-blue-100 bg-white p-4 font-sans text-sm leading-7 text-slate-800 shadow-inner">
          {value}
        </pre>
      ) : null}
    </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between print:hidden">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-slate-600">
            생성된 초안은 관리자가 직접 수정할 수 있으며, 현재 내용은 Supabase AI 결과 저장소(ai_outputs)에 저장됩니다.
          </p>
        </div>
        <button
...
function AiOutputHistorySection({
  deletingId,
  filter,
  isLoading,
  onCopy,
  ...
  filter: "all" | AiOutputType;
  isLoading: boolean;
  onCopy: (content: string) => Promise<void>;
  onDelete: (output: AiOutputHistory) => void;
  onFilterChange: (value: "all" | AiOutputType) => void;
  onRefresh: () => void;
  onSearchChange: (value: string) => void;
  onSelect: (id: string) => void;
  outputs: AiOutputHistory[];
  search: string;
  ...
  const filteredOutputs = outputs.filter((output) => {
    if (filter !== "all" && output.output_type !== filter) {
      return false;
    }
    ...
    return [output.title, output.content].map(normalizeSearchText).join(" ").includes(keyword);
  });
  const selectedOutput =
    filteredOutputs.find((output) => output.id === selectedId) ?? filteredOutputs[0] ?? null;
  ...
  async function copySelectedOutput() {
    ...
    try {
      await onCopy(selectedOutput.content);
      setCopiedId(selectedOutput.id);
      window.setTimeout(() => setCopiedId(null), 2000);
    } catch {
      window.alert("AI 결과 복사에 실패했습니다. 브라우저 권한을 확인해 주세요.");
    }
  }

  function printSelectedOutput() {
    const printStyle = document.createElement("style");
    printStyle.setAttribute("data-ai-output-history-print", "true");
    printStyle.textContent = `
      @media print {
        body * { visibility: hidden !important; }
        #ai-output-history-print-area, #ai-output-history-print-area * { visibility: visible !important; }
        #ai-output-history-print-area { position: absolute; inset: 0; width: 100%; padding: 24px; background: white; }
      }
    `;
    document.head.appendChild(printStyle);
    window.print();
  ...
  return (
    <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-base font-black text-slate-900">AI 호출 이력</h2>
          <p className="mt-1 text-xs font-semibold text-slate-500">
            Supabase AI 결과 저장소(ai_outputs)에 저장된 과거 결과입니다.
          </p>
        </div>
        <button
          className="w-fit rounded-full border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-700 transition hover:border-navy hover:text-navy disabled:cursor-not-allowed disabled:opacity-60"
          disabled={isLoading}
          onClick={onRefresh}
          type="button"
        >
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[220px_1fr]">
        <select
          ...
          onChange={(event) => onFilterChange(event.target.value as "all" | AiOutputType)}
          value={filter}
        >
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,360px)_1fr]">
        <div className="space-y-2">
          {isLoading ? (
            <p className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm font-bold text-blue-700">
              AI 호출 이력을 불러오고 있습니다.
            </p>
          ) : null}
          ...
          {filteredOutputs.map((output) => (
            <div
              className={`rounded-xl border p-3 transition ${
                selectedOutput?.id === output.id
                  ? "border-navy bg-navy/5 ring-2 ring-navy/10"
            ...
            >
              <button className="w-full text-left" onClick={() => onSelect(output.id)} type="button">
                ...
                <p className="mt-2 break-words text-sm font-black text-slate-900">{output.title || "제목 없음"}</p>
                <p className="mt-1 text-xs font-semibold text-slate-500">모델: {output.model || "-"}</p>
                <p className="mt-2 line-clamp-3 break-words text-xs leading-5 text-slate-600">
                  {createTextPreview(output.content)}
                </p>
              </button>
              <button
                className="mt-3 rounded-full border border-red-200 bg-white px-3 py-1 text-xs font-black text-red-600 transition hover:border-red-400 disabled:cursor-not-allowed disabled:opacity-60"
                disabled={deletingId === output.id}
                onClick={() => onDelete(output)}
                type="button"
              >
                {deletingId === output.id ? "삭제 중..." : "삭제"}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          {selectedOutput ? (
            <>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between print:hidden">
                <div>
                  <p className="text-sm font-black text-slate-900">{selectedOutput.title || "AI 결과"}</p>
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    {selectedOutput.output_type} · {selectedOutput.model || "-"} · {formatDate(selectedOutput.created_at)}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
                    className="rounded-full border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-700 transition hover:border-navy hover:text-navy"
                    onClick={copySelectedOutput}
                    type="button"
                  >
                    {copiedId === selectedOutput.id ? "복사됨" : "복사"}
                  </button>
                  <button
                    className="rounded-full border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-700 transition hover:border-navy hover:text-navy"
                    onClick={printSelectedOutput}
                    type="button"
                  >
                    PDF 출력
                  </button>
              ...
              <div className="mt-4" id="ai-output-history-print-area">
                <pre className="max-h-[640px] overflow-auto whitespace-pre-wrap break-words rounded-xl border border-slate-200 bg-white p-4 font-sans text-sm leading-7 text-slate-800 shadow-inner print:max-h-none print:border-0 print:shadow-none">
                  {selectedOutput.content || ""}
                </pre>
              </div>
            </>
          ) : (
            <p className="text-sm font-semibold text-slate-600">왼쪽 목록에서 AI 결과를 선택해 주세요.</p>
          )}
        </div>
      </div>
    </section>
...
function createTextPreview(value?: string | null) {
